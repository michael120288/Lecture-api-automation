...
// Shared Axios options — bypasses rate limit and always returns response object
const TEST_OPTS = {
...
beforeAll(async () => {
  const loginRes = await axios.post(signinUrl, credentials, TEST_OPTS);
...
describe('1. Forgot password', () => {
  ...
  it('valid email returns 200', async () => {
    ...
    // 200 if email exists, 400 if not, 500 if email service is disabled
  ...
  it('missing email returns 400', async () => {
    const res = await axios.post(forgotPwUrl, {}, TEST_OPTS);
    expect(res.status).toBe(400);
  });
...
describe('2. Reset password — validation errors', () => {
  ...
  it('mismatched passwords returns 400', async () => {
    ...
    expect(res.status).toBe(400);
    expect(res.data.message).toContain('match');
  });
  ...
  it('empty body returns 400', async () => {
    const res = await axios.post(resetPwUrl('anytoken'), {}, TEST_OPTS);
    expect(res.status).toBe(400);
  });
...
describe('3. SSO — Single Sign-On', () => {
  ...
  it('valid JWT returns 200 and user object', async () => {
    const res = await axios.post(ssoUrl, { token: jwt }, TEST_OPTS);
  ...
  it('empty body returns 400 "Token required"', async () => {
    const res = await axios.post(ssoUrl, {}, TEST_OPTS);
    expect(res.status).toBe(400);
    expect(res.data.message).toBe('Token required');
  });

  it('invalid JWT string returns 400 or 500', async () => {
    const res = await axios.post(ssoUrl, { token: 'not.a.valid.jwt' }, TEST_OPTS);
    // 400 when JWT validation catches the error, 500 when it reaches an unhandled path
